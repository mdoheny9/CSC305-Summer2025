...
#### Ray Tracing a Sphere [2pt]

1. Modify the ray-sphere intersection to follow the generic case we saw in class.

#### Ray Tracing a Parallelogram [8pt]

2. Set up the parameters of the parallelogram (position of the corner, plus one vector for each side of the parallelogram)
3. Create a function to check if a ray intersects with an arbitrary parallelogram.
4. Calculate the intersection between the ray and the parallelogram using the function defined above.
5. Compute the normal of that intersection point.



#### Ray Tracing with Perspective Projection [6pt]
